...
def accessible(movies: list[data.Movie]) -> list[data.Movie]:
    movies_list = []
    for movie in movies:
        subtitle = movie.subtitles.get("En")
...
def children_movies(movies: list[data.Movie]) -> list[data.Movie]:
    sorted_list = []
    for movie in movies:
        if movie.maturity.get("G") or movie.maturity.get("PG"):
...
def young_adult_movies(movies: list[data.Movie]) -> list[data.Movie]:
    sorted_list = []
    for movie in movies:
        if movie.maturity.get("G") or movie.maturity.get("PG") or movie.maturity.get("PG-13"):
            sorted_list.append(movie)
    return sorted_list
...
def sorted_by_genre(movies: list[data.Movie], type: str) -> list[data.Movie]:
